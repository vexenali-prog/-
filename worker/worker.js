...
// Торгуемые монеты (ядро + прошедшие скрининг) и полный список наблюдения.
const TRADED = [
  "BTC-USDT", "ETH-USDT", "SOL-USDT", "XRP-USDT", "DOGE-USDT", "TRX-USDT",
  "SUI-USDT", "ARB-USDT", "XLM-USDT", "XAUT-USDT",
];
const SYMBOLS = [
  ...TRADED,
...
async function viewMenu(env) {
  ...
  return {
    text:
      ...
      "Торговля 10 инструментами (9 криптовалют + золото XAUT) по трендовой модели, проверенной на " +
      "многолетних исторических данных. Управление риском: " +
      "стоп-лосс, трейлинг-стоп, фильтры рыночного режима и объёма.\n\n" +
...
async function viewPortfolio(env) {
  ...
  return {
    text:
      ...
      (live ? "\n\n<i>Цены — прямо сейчас с биржи. Время — по Москве.</i>" : ""),
    keyboard: kb([
...
function viewHelp(arg) {
  if (arg === "strategy") {
    return {
      text:
        ...
        "Торгую 10 инструментов: ядро BTC, ETH, SOL, XRP, DOGE, TRX (на 4 годах " +
        "истории +237%) плюс SUI, ARB, XLM и золото XAUT — каждый прошёл скрининг из десятков " +
        "кандидатов и усиливает корзину. Остальные наблюдаю, но не торгую.\n\n" +
        "🛡 <b>Защита денег</b>:\n" +
        ...
        "• фильтр объёма — вход только на движении, подтверждённом объёмом\n" +
        "• пирамидинг — докупка при +15%: усиливаю то, что доказало рост\n\n" +
        "Проверка на истории: ядро портфеля за 4,2 года (обвал 2022-го, " +
        "крах FTX, бычий 2024-й) — <b>+191%</b>; корзина из 9 монет за " +
        "2 года — <b>+117%</b>. Просадки до −28% были и будут — к ним " +
        "надо быть готовым.\n\n" +
  ...
  if (arg === "schedule") {
    return {
      text:
        ...
        "• Дневная сводка — каждое утро ~10:00 по Москве\n" +
  ...
  return {
    text:
      ...
      "капитал, реальные рыночные цены). Торгуются 9 активов: BTC, ETH, " +
      "SOL, XRP, DOGE, TRX, SUI, ARB, XLM; ещё 14 под наблюдением.\n\n" +
...
со стоп-лоссом, трейлинг-стопом, фильтрами рыночного режима и объёма. Обновлено: ${upd} (МСК).
